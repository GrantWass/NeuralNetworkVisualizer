...
export const getLossCallout = (
  ...
  if (pct > 2) return { text: `Loss decreased ${pct.toFixed(1)}% — steady progress. Keep training.`, type: 'good' };
  if (pct >= -1) return { text: `Loss barely changed (${pct.toFixed(1)}%). The model may be stuck — try a higher learning rate or run more cycles.`, type: 'slow' };
  return { text: `Loss increased ${Math.abs(pct).toFixed(1)}%. The learning rate may be too high — try lowering it.`, type: 'bad' };
};
